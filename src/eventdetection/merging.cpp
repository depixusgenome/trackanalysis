...
namespace eventdetection { namespace merging {
    namespace {
        float get0(INTERVALS const & intr, size_t i) { return intr[i].first; }
        float get1(INTERVALS const & intr, size_t i) { return intr[i].second; }
    }
    ...
    namespace statsmerging
    {
        ...
        template <typename T>
        inline auto initstats(T const & self, float const *data, INTERVALS const & intervals)
        {
        ...
        template <typename T>
        void run(T const & self, float const *data, INTERVALS & intervals)
        {
            if(intervals.size() <= 1)
                return;
    ...
    void HeteroscedasticEventMerger::run(float const *data, INTERVALS & intervals) const
    ...
    void PopulationMerger::run(float const * data, INTERVALS & intervals) const
    {
    ...
    void ZRangeMerger::run(float const * data, INTERVALS & intervals) const
    {
        using namespace rangemerging;
    ...
    void  MultiMerger::run(float const *data,  INTERVALS & intervals) const
    {
