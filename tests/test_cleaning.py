...
def test_datacleaning():
    ...
    bead = np.concatenate([
        np.random.normal(.1, 3e-3,  100), np.random.normal(1.1, 3e-3, 100),
...
def test_subtract_med():
    "tests subtractions"
    _    = dict(dtype = 'f4')
    agg  = lambda x: SubtractMedianSignal.apply([i.astype('f4') for i in x], (0,5))  # noqa
    assert_allclose(agg([np.arange(5, **_)]*5),           np.arange(5, **_))
    assert_allclose(agg([np.arange(5, **_), np.ones(5, **_)]), np.arange(5, **_)*.5+.5)
...
def test_cleaningview(view, bokehaction):
    "test the view"
    server = bokehaction.start(f'cleaning.view.{view}CleaningView', 'taskapp.toolbar')
...
@integrationmark
@pytest.mark.parametrize("view", ['Bead', ''])
def test_undersampling(view, bokehaction):
    "test the view"
    server = bokehaction.start(
        f'cleaning.view.{view}CleaningView',
        'taskapp.toolbar',
        runtime = 'selenium')
    server.load('big_legacy')
...
def test_cycletable(bead: int, taskcount: int):
    ...
    find = lambda tpe: next((i for i in tasks if isinstance(i, tpe)), None)  # noqa
