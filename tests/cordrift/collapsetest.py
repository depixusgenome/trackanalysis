...
def test_collapse_by_merging():
    ...
    assert all(prof.count == 3)
    truth = numpy.mean(yvals[5:10,1:-1] - numpy.mean(yvals[5:10,1:-1], axis = 0), axis = 1)
    numpy.testing.assert_allclose(truth-truth[0], prof.value-prof.value[0], rtol = 1e-4)

    # test non-overlapping intervals
    inters[0] = Range(15, yvals[15:25,1])
    prof      = inst(iter(inters[:-1]))
    assert all(prof.count == ([3]*5+[0]*5+[1]*10))
    numpy.testing.assert_allclose(truth-truth[0], prof.value[:5]-prof.value[0], rtol = 1e-5)
    assert all(prof.value[5:] == 0.)
